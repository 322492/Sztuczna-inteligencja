...
class Deck:
    ...
    def show(self):
        res = [c.to_string() for c in self.deck]
        print(res)
...
class Game:
    ...
    def show_hands(self):
        for i in range(self.number_of_players):
            res = [c.to_string() for c in self.hands[i]]
            print(i, len(res), res)

    def turn(self):
        cards, (amount, value) = self.players[self.player].move(self.history, self.player, self.hands[self.player], self.pile) # cards = [c1, ..., cn]
...
class Little_calculator(Player):
    ...
    def move(self, history, player, hand, pile):
        ...
        if not poss: # have to lie
            ...
            res = random.sample(hand, number_of_cards)
            ...
            return res, (number_of_cards, best_value)
...
class Kid(Player):
    ...
    def move(self, history, player, hand, pile):
        ...
        if want_to_lie or not poss:
            ...
            return res, (number_of_cards, random.choice(options))
        else:
            lower_val = 'A'
            card = poss[0]
            for c in poss:
...
if __name__ == '__main__':
    p1 = Random()
    p2 = Random(challenging = 0.1)
    p3 = Random(challenging = 0.1)
    p4 = Random(challenging = 0.1)
    p5 = Beginner()
    p6 = Beginner()
    pb = Beginner()
    p7 = Little_calculator()
    p8 = Little_calculator()
    p9 = Little_calculator()
    plc = Little_calculator()
    p11 = Kid()
    p12 = Kid()
    p13 = Kid()
    ...
    print(f"Draws after {MAX_NUMBER_OF_TURNS} moves: {res[-1]}")
    for i in range(len(players)):
        print(f"Wins of player {i} {players[i].introduce()}: {res[i]}, what is {res[i] * 100 / NUMBER_GAMES}%")